...
The review gate drops it because no upstream evidence supports it. That is the point of the demo.
...
```text
Request -> Intake -> Risk -> Plan -> Review Gate -> Decision Writer -> Run Packet
```

Each agent output is validated with Zod before it reaches the next step.
...
Failure mode shown visually:

![Failure mode walkthrough](assets/failure-mode.png)
